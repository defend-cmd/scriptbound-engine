...
public class DashboardScreen extends Screen
{
    ...
    private java.util.List<String> uiList = new java.util.ArrayList<>();
    private final java.util.List<UiRect> uiRowRects = new java.util.ArrayList<>();
    private final java.util.List<UiRect> uiDelRects = new java.util.ArrayList<>();
    private UiRect uiNewRect = rect();
    ...
    private void renderContent(GuiGraphics graphics, int mouseX, int mouseY)
    {
        if (this.activeTab == Tab.UI)
        {
            ...
            renderUiScreen(graphics, mouseX, mouseY);
            return;
        }
    ...
    public void setUiList(java.util.List<String> list)
    {
        this.uiList = list == null ? new java.util.ArrayList<>() : list;
    }

    private void renderUiScreen(GuiGraphics graphics, int mouseX, int mouseY)
    {
        int x = dev.scriptbound.client.ui.UiScale.sidebarW() + dev.scriptbound.client.ui.UiScale.pad();
        int y = 44;
        int w = this.width - x - dev.scriptbound.client.ui.UiScale.pad();
        int h = this.height - y - dev.scriptbound.client.ui.UiScale.pad();

        dev.scriptbound.client.ui.UiDraw.panel(graphics, x, y, w, h);
        dev.scriptbound.client.ui.UiDraw.panelHeader(graphics, this.font, x, y, w, "UI Documents");

        this.uiNewRect = new UiRect(x + w - 70, y + 3, 64, 16);
        dev.scriptbound.client.ui.UiDraw.button(graphics, this.font, this.uiNewRect, "+ New", null, this.uiNewRect.contains(mouseX, mouseY), true, true);

        int hh = dev.scriptbound.client.ui.UiTheme.PANEL_HEADER_H;
        graphics.drawString(this.font, "Click a document to edit it. [x] deletes.", x + 8, y + hh + 4, dev.scriptbound.client.ui.UiTheme.TEXT_DIM);

        this.uiRowRects.clear();
        this.uiDelRects.clear();
        int ry = y + hh + 18;

        if (this.uiList.isEmpty())
        {
            graphics.drawString(this.font, "No UI documents yet — press + New.", x + 8, ry, dev.scriptbound.client.ui.UiTheme.TEXT_DIM);
            return;
        }

        for (String id : this.uiList)
        {
            UiRect row = new UiRect(x + 6, ry, w - 12, 16);
            this.uiRowRects.add(row);
            ...
            graphics.drawString(this.font, id, row.x() + 6, row.y() + 4, dev.scriptbound.client.ui.UiTheme.TEXT);

            UiRect del = new UiRect(row.x() + row.w() - 16, row.y(), 16, 16);
            this.uiDelRects.add(del);
            graphics.drawString(this.font, "x", del.x() + 5, del.y() + 4, del.contains(mouseX, mouseY) ? 0xFFFF5555 : dev.scriptbound.client.ui.UiTheme.TEXT_DIM);

            ry += 17;
        }
    }

    private boolean handleUiClick(double mouseX, double mouseY, int button)
    {
        if (button != 0)
        {
            return true;
        ...
        if (this.uiNewRect.contains(mouseX, mouseY))
        {
            NetworkHandler.CHANNEL.sendToServer(new dev.scriptbound.network.packet.OpenUiEditorPacket("ui_" + (this.uiList.size() + 1)));
            return true;
        }

        for (int i = 0; i < this.uiDelRects.size(); i++)
        {
            if (this.uiDelRects.get(i).contains(mouseX, mouseY))
            {
                NetworkHandler.CHANNEL.sendToServer(new dev.scriptbound.network.packet.DeleteUiPacket(this.uiList.get(i)));
                return true;
            }
        }

        for (int i = 0; i < this.uiRowRects.size(); i++)
        {
            if (this.uiRowRects.get(i).contains(mouseX, mouseY))
            {
                NetworkHandler.CHANNEL.sendToServer(new dev.scriptbound.network.packet.OpenUiEditorPacket(this.uiList.get(i)));
                return true;
            }
        }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button)
    {
        ...
        if (this.renameModalOpen) {
            if (this.renameConfirmRect.contains(mouseX, mouseY) && !this.renameField.value().isBlank()) {
                ...
                if (!newName.isBlank() && !newName.equals(this.listContextId)) {
                    dev.scriptbound.network.NetworkHandler.CHANNEL.sendToServer(new dev.scriptbound.network.packet.DashboardActionPacket("rename_doc", categoryForTab() + "|" + this.listContextId, newName));
        ...
        if (button == 0)
        {
            ...
            for (int i = 0; i < Tab.values().length; i++)
            {
                if (this.tabRects[i].contains(mouseX, mouseY))
                {
                    ...
                    if (this.activeTab == Tab.UI)
                    {
                        NetworkHandler.CHANNEL.sendToServer(new dev.scriptbound.network.packet.RequestUiListPacket());
                    }
        ...
        if (this.activeTab == Tab.UI)
        {
            return handleUiClick(mouseX, mouseY, button);
        }
        ...
        if (this.listContextMenuOpen) {
            ...
            if (this.listContextDuplicateRect.contains(mouseX, mouseY)) {
                NetworkHandler.CHANNEL.sendToServer(new DashboardActionPacket("duplicate_doc", categoryForTab() + "|" + this.listContextId));
                this.listContextMenuOpen = false;
                return true;
            }
            if (this.listContextDeleteRect.contains(mouseX, mouseY)) {
                NetworkHandler.CHANNEL.sendToServer(new DashboardActionPacket("delete_doc", categoryForTab() + "|" + this.listContextId));
                if (this.listContextId.equals(this.selectedId)) {
                    this.selectedId = null;
                }
                this.listContextMenuOpen = false;
        ...
        {
            dev.scriptbound.network.NetworkHandler.CHANNEL.sendToServer(new DashboardActionPacket("reload", ""));
            return true;
        }
        ...
        {
            dev.scriptbound.network.NetworkHandler.CHANNEL.sendToServer(new DashboardActionPacket("refresh", ""));
            requestDocumentLoad();
            return true;
        }
    ...
    private void requestDocumentLoad()
    {
        ...
        dev.scriptbound.network.NetworkHandler.CHANNEL.sendToServer(new DashboardActionPacket(
            "load_doc",
    ...
    private void runSelected()
    {
        ...
        dev.scriptbound.network.NetworkHandler.CHANNEL.sendToServer(new DashboardActionPacket(action, this.selectedId));
    }
    ...
    private void createNewGraphDoc()
    {
        ...
        NetworkHandler.CHANNEL.sendToServer(new DashboardActionPacket("create_doc", category + "|" + id));
        commitCurrentEdits();
        this.selectedId = id;
        this.searchField.setValue("");
        this.searchField.setFocused(false);
        ...
        if (this.searchField.keyPressed(key) || this.valueField.keyPressed(key) || this.newIdField.keyPressed(key) || this.renameField.keyPressed(key))
        {
            if (key == org.lwjgl.glfw.GLFW.GLFW_KEY_ENTER || key == org.lwjgl.glfw.GLFW.GLFW_KEY_KP_ENTER)
            {
                ...
                else if (this.renameField.focused() && this.renameModalOpen && !this.renameField.value().isBlank())
                {
                    String newName = sanitizeId(this.renameField.value());
                    if (!newName.isBlank() && !newName.equals(this.listContextId)) {
                        NetworkHandler.CHANNEL.sendToServer(new DashboardActionPacket("rename_doc", categoryForTab() + "|" + this.listContextId, newName));
                        if (this.listContextId.equals(this.selectedId)) {
                            this.selectedId = newName;
                        }
                    }
