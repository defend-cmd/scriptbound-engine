...
public final class FlowGraphEditor
{
    ...
    private boolean handlePropsClick(double mouseX, double mouseY, int button)
    {
        ...
        String[][] defs = propDefs(node.type);
        int x = this.propsRect.x() + 8;
        int y = this.propsRect.y() + UiTheme.PANEL_HEADER_H + 8 + 16;
        int w = this.propsRect.w() - 16;
        boolean any = false;
        ...
        for (int i = 0; i < defs.length && i < this.fields.length; i++)
        {
            ...
            boolean focused = this.fields[i].mouseClicked(new UiRect(x, y + 10, w, 18), mouseX, mouseY, button);
            any = any || focused;
            y += 34;
        }
    ...
    private static String[][] propDefs(String type)
    {
        return switch (type)
        {
            ...
            case "ui_open" -> new String[][] { {"ui", "UI id"} };
            case "ui_close" -> new String[][] { {"ui", "UI id (blank = all)"} };
            case "ui_set" -> new String[][] { {"ui", "UI id"}, {"element", "Element id"}, {"field", "Field (text/value/visible)"}, {"value", "Value"} };
