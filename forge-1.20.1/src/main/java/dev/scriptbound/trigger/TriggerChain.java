...
public final class TriggerChain
{
    ...
    public static TriggerAction parseActionBody(JsonObject entry)
    {
        ...
        return switch (entry.get("type").getAsString().toLowerCase())
        {
            ...
            case "ui_open" -> dev.scriptbound.trigger.actions.UiOpenAction.fromJson(entry);
            case "ui_close" -> dev.scriptbound.trigger.actions.UiCloseAction.fromJson(entry);
            case "ui_set" -> dev.scriptbound.trigger.actions.UiSetAction.fromJson(entry);
            default -> {
