...
public final class ScriptBoundPaths
{
    ...
    public static Path uiDir(MinecraftServer server)
    {
        return root(server).resolve("ui");
    }

    public static Path uiFile(MinecraftServer server, String id)
    {
        return uiDir(server).resolve(id + ".json");
    }
}
